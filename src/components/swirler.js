...
class Swirler extends Component {
  componentDidMount() {
    ...
    class Application {
      ...
      set clearOnRedraw(value) {
        if (
          [Application.NOCLEAR, Application.CLEAR, Application.FADE].indexOf(
            value
          ) > -1
        ) {
    ...
    class Tracer extends Actor {
      ...
      onAnimate(e) {
        let force = this.field
          .solveForPosition(this.position)
          .multiplyScalar(0.011);
        ...
        if (this.position.x < -this.dimensions.width) {
          ...
          this.position.x >
          app.dimensions.width + this.dimensions.width
        ) {
        ...
        if (this.position.y < -this.dimensions.height) {
          ...
          this.position.y >
          app.dimensions.height + this.dimensions.height
        ) {
  ...
  render() {
    return <canvas className="drawer" />;
  }
}
